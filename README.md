...
│   │       └── chapter_map.json           # Chapter별 단어 구조
│   │
│   └── mapped/                        # 단어-기출 매핑 결과
...
| `extract_chapters.py` | 노랭이 PDF에서 Chapter 구조 + 900점 단어 추출 | `data/raw/hackers_vocab.pdf` → `data/processed/vocab/chapter_map.json` | **활성** |
...
### `scripts/anki/` — Anki 덱 생성 (Phase 3)
...
Chapter별 단어 구조. chapter, title, words[] 구조.
...
프로젝트 전역 설정. 경로, ETS 권/파트 번호, Anki 덱 이름/ID, Part5 카테고리 목록 정의.

### `.gitignore`

`data/raw/`, `output/anki/*.apkg`, `.secret/`, `.request/` 등 미추적.
